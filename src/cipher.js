  ...
  encode: (offset, string) => {
    ...
    for (let i = 0; i < string.length; i += 1) {
      ...
      if (numberCodeAscii >= 65 && numberCodeAscii <= 90) {
        resultadoCipher += String.fromCharCode(((numberCodeAscii - 65 + parseInt(offset, 10)) % 26) + 65);
      } else if (numberCodeAscii >= 97 && numberCodeAscii <= 122) {
        resultadoCipher += String.fromCharCode(((numberCodeAscii - 97 + parseInt(offset, 10)) % 26) + 97);
      } else if (numberCodeAscii >= 48 && numberCodeAscii <= 57) {
        resultadoCipher += String.fromCharCode(((numberCodeAscii - 48 + parseInt(offset, 10)) % 10) + 48);
      } 
      else { 
        resultadoCipher += string.charAt(i)
      }
  ...
  decode: (offset, string) => {
    ...
    for (let i = 0; i < string.length; i += 1) {
      ...
      if (numberCodeAscii >= 65 && numberCodeAscii <= 90) {
        ...
        resultadoDecifrado += string.charAt(i); 
      }
